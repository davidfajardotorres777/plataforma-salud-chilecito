...
def test_get_mongo_config_with_env():
    with patch('os.getenv', side_effect=mock_getenv({"MONGO_URI": "mongodb://remote:27017", "DB_NAME": "test_db"})):
        config = config_vars.get_mongo_config()
...
def test_get_redis_config_with_env():
    with patch('os.getenv', side_effect=mock_getenv({"REDIS_HOST": "redis.server", "REDIS_PORT": "6380", "REDIS_DB": "1", "REDIS_PASSWORD": "secretpassword"})):
        config = config_vars.get_redis_config()
...
def test_get_email_config_with_env():
    with patch('os.getenv', side_effect=mock_getenv({"SMTP_SERVER": "smtp.test.com", "SMTP_PORT": "465", "SMTP_USERNAME": "test_user", "SMTP_PASSWORD": "test_password", "FROM_EMAIL": "test@test.com"})):
        config = config_vars.get_email_config()
...
def test_get_app_config_with_env():
    with patch('os.getenv', side_effect=mock_getenv({"SECRET_KEY": "supersecret", "BASE_URL": "https://api.test.com", "ENVIRONMENT": "production"})):
        config = config_vars.get_app_config()
